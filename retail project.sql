...
--1. find null value in each column
select * from retail
where
...
-- 2. Delete row which contain null value
 Delete from retail
  where 
     transactions_id is null
     or
 ...
 -- How many unique category we have?
  select count(distinct category) as total_catagory from retail;
...
-- Q.6 Write a SQL query to find the total number of transactions (transaction_id) made by each gender in each category.
       select category, gender,count(*) as total_trans from retail
